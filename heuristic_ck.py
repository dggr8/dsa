...
def possible_moves(current_position,visited,size):
	...
	###HEURISTIC UPDATE###
	moves_list = moves_stack.getList()
	heuristic_num = [None]*len(moves_list)
	for i in range(0,len(moves_list)):
		heuristic_num[i] = n_possible_moves(moves_list[i],visited,size)

	moves_sorted_stack = s.Stack()

	for i in range(0,len(moves_list)):
		for j in range(0,len(moves_list)):
			if heuristic_num[j] == min(heuristic_num):
				moves_sorted_stack.push(moves_list[j])
				heuristic_num[j] = 100 #INF
				break
		
	return moves_sorted_stack

def n_possible_moves(current_position,visited,size):
	all_combinations = [[1,2],[1,-2],[2,1],[2,-1],[-1,2],[-1,-2],[-2,1],[-2,-1]]
	all_moves = []
	for comb in all_combinations:
		all_moves.append(current_position.getNext(comb[0],comb[1]))
